...
met <- read_csv("Tortuguero_summer2024.csv", col_types = cols(Timestamp = col_datetime(format = "%Y-%m-%d %H:%M:%S")))
...
# Initialize Excel and store column names
Excel <- data.frame()   # start empty
Colnames <- names(Excel)  # may be NULL at first — better to define after 1st result
...
for (i in unique_dates) {
  ...
  Wetland <- "TO"  # Or extract from filename or metadata if dynamic
...
Excel$NEP <- Excel$GPP.50 + Excel$ER.50

write.csv(Excel, "met_model_output.csv", row.names = FALSE)

ggplot(Excel, aes(x=Date, y=GPP.50)) +
  geom_point() +
  geom_hline(yintercept = 0)

ggplot(Excel, aes(x=Date, y=ER.50)) +
  geom_point() +
  geom_hline(yintercept = 0)

ggplot(Excel, aes(x=Date, y=NEP)) +
  geom_point() +
  geom_hline(yintercept = 0)
...
# ***
# identify subset of data for this model run ** from 00:00 to 02:00 the next day **
data=met[met$dtime>=as.numeric(chron(dates="09/30/2023", times="00:00:00")) & met$dtime<=as.numeric(chron(dates="10/01/2023", times="00:02:00")),]
plot(data$dtime, data$oxy)
plot(data$dtime, data$temp)
plot(data$dtime, data$light) # re-check against met data; leave for now
