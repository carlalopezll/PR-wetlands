...
mout <- function (data, met.out){
  mout <- length(24)
  mout[1] <- Wetland
  mout[2] <- as.character(data$Timestamp[50])
...
radi <- function(deg) deg * pi/180

# Specifically for Puerto Rico
lightest <- function(time,
                     lat     = 18.2,
                     longobs = 66.5,
                     longstd = 60) {
  
  jday <- yday(time)  # day-of-year
  
  # Equation of time correction
  E <- 9.87 * sin(radi(720 * (jday - 81) / 365)) -
    7.53 * cos(radi(360 * (jday - 81) / 365)) -
    1.5  * sin(radi(360 * (jday - 81) / 365))
  
  # fractional day between 0 and 1
  frac_day <- (hour(time)*3600 +
                 minute(time)*60 +
                 second(time)) / 86400
  
  # solar time
  ST <- frac_day +
    (3.989/1440) * (longstd - longobs) +
    E/1440
  ...
  theta <- acos(
    sin(radi(solardel)) * sin(radi(lat)) +
      cos(radi(solardel)) * cos(radi(lat)) * cos(radi(hourangle))
  )
  
  suncos <- pmax(cos(theta), 0)  # zero when sun is below horizon
  GI <- suncos * 2326
