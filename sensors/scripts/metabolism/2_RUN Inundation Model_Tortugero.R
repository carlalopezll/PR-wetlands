...
met <- read_csv("sensors/data/DO/Summer 2024/Tortuguero/Tortuguero_summer2024.csv", col_types = cols(Timestamp = col_datetime(format = "%Y-%m-%d %H:%M:%S")))
...
write.csv(Excel, "TO_met_model_output.csv", row.names = FALSE)

Excel <- read_csv("sensors/data/TO_met_model_output.csv")
...
ggsave("sensors/Tortuguero metabolism.jpg")
